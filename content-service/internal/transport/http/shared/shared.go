...
func ErrorEncoder(ctx context.Context, err error, w http.ResponseWriter) {
	...
	// Populate app-session fields so request finalizer logs are informative.
	if v := ctx.Value(ctxYB.AppSession); v != nil {
		if ctxSess, ok := v.(*ctxYB.Context); ok && ctxSess != nil {
